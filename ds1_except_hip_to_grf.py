"""
Downstream Task 1: Hip-masked kinematics to GRF estimation
Estimate ground reaction forces with hip joint angles masked (unknown).
This demonstrates the model's robustness to missing kinematic data.
"""
...
# import 시 문제가 생기지 않도록 프로젝트 경로들을 sys.path에 추가해줌
...
def get_hip_column_indices(opt):
    """Get column indices for hip joints (both left and right)."""
    hip_cols = []
    for i, col_name in enumerate(opt.model_states_column_names):
        # Hip joint angles are represented as 6D rotation vectors
        if 'hip_r_' in col_name or 'hip_l_' in col_name:
            hip_cols.append(i)
    
    print(f"\nHip joint columns to mask ({len(hip_cols)} total):")
    for idx in hip_cols:
        print(f"  [{idx}] {opt.model_states_column_names[idx]}")
    
    return hip_cols


def estimate_grf_with_masked_hip(opt, model_states_windows, hip_cols):
    """
    Two-stage GRF estimation with masked hip joints:
    1. Diffusion model inpaints missing hip kinematics
    2. Refinement model estimates GRF from complete kinematics
    """
    # Stage 1: Load diffusion model for inpainting
    print("\nStage 1: Loading GaitDynamicsDiffusion model for hip inpainting...")
    diffusion_model = MotionModel(opt)
    diffusion_model.eval()
    
    # Create mask: all kinematics except hip are known, all kinetics are unknown
    masks = torch.zeros_like(model_states_windows)
    for i in opt.kinematic_diffusion_col_loc:
        if i not in hip_cols:
            masks[:, :, i] = 1.0
    
    print(f"\nInput shape: {model_states_windows.shape}")
    print(f"Known kinematic channels: {masks[:, :, opt.kinematic_diffusion_col_loc].sum(dim=-1).max().item():.0f}")
    print(f"Masked (hip) kinematic channels: {len(hip_cols)}")
    
    # Inpaint missing hip kinematics
    print("Inpainting missing hip kinematics using diffusion model...")
    state_with_inpainted_hip = diffusion_model.eval_loop(
        opt,
    ...
    # state_with_inpainted_hip shape: [1, N, window_len, channels]
    state_with_inpainted_hip = state_with_inpainted_hip[0]
    
    # Stage 2: Load refinement model for force estimation
    print("\nStage 2: Loading GaitDynamicsRefinement model for GRF estimation...")
    refinement_model = BaselineModel(opt, TransformerEncoderArchitecture)
    
    # Create mask for refinement: all kinematics known (including inpainted hip), kinetics unknown
    masks_refinement = torch.zeros_like(state_with_inpainted_hip)
    masks_refinement[:, :, opt.kinematic_diffusion_col_loc] = 1.0
    
    # Predict forces
    print("Estimating GRF from inpainted full-body kinematics...")
    state_pred = refinement_model.eval_loop(
        opt,
        state_with_inpainted_hip,
        masks_refinement,
        num_of_generation_per_window=1,
        mode="inpaint"
    )
    ...
    return state_pred, state_with_inpainted_hip
...
def plot_grf_comparison(measured_states, predicted_states, opt, start_indices, sampling_rate, save_path):
    ...
    fig.suptitle('GRF Estimation: Masked Hip Kinematics → Inpainting → Forces', fontsize=16, fontweight='bold')
    ...
    for row, (foot_cols, foot_label) in enumerate([(r_cols, 'Right Foot'), (l_cols, 'Left Foot')]):
        for col_idx, kinetic_idx in enumerate(foot_cols[:3]):  # Force components
            ...
            ax.plot(time, predicted, 'r--', linewidth=1.5, label='Predicted (Hip Masked)', alpha=0.7)
    ...
    print("\n" + "="*80)
    print("GRF ESTIMATION SUMMARY (Hip Joints Masked)")
    print("="*80)
...
def main():
    parser = argparse.ArgumentParser(description='Downstream Task 1: Hip-masked kinematics to GRF')
    ...
    # Get hip column indices
    hip_cols = get_hip_column_indices(opt)
    
    # Estimate GRF with masked hip joints (two-stage: inpainting + refinement)
    predicted_states, inpainted_states = estimate_grf_with_masked_hip(
        opt, 
        model_states_windows, 
        hip_cols
    )
    ...
    save_path = os.path.join(script_args.save_dir, f'{trial_name}_except_hip.png')
