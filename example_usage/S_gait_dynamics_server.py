        ...
        for i_trial in range(start_trial, end_trial):
            ...
            for col in trial_.missing_col:
                try:
                    ...
                    # 컬럼이 model_states_column_names에 없으면 무시
                    print(f"Warning: Column '{col}' not found in model_states_column_names, skipping...")
                    continue
...
def usr_inputs():
    ...
    print("🚀 GaitDynamics 서버 버전 실행")
    print("=" * 60)

    # .mot 파일들을 상위 디렉토리에서 찾기
    parent_dir = os.path.dirname(opt.subject_data_path)
    file_paths = []
    ...
    if len(file_paths) == 0:
        raise RuntimeError(f'No .mot file found in {parent_dir}')
    
    print(f"찾은 .mot 파일들: {[os.path.basename(f) for f in file_paths]}")
    opt.file_paths = file_paths

    # .osim 파일을 현재 디렉토리에서 찾기
    osim_paths = []
    ...
    if len(osim_paths) == 0:
        # 기본 예제 모델 사용
        opt.subject_osim_model = os.path.join(opt.subject_data_path, 'example_opensim_model.osim')
        print(f"기본 OpenSim 모델 사용: example_opensim_model.osim")
    else:
        opt.subject_osim_model = osim_paths[0]
        print(f"OpenSim 모델: {os.path.basename(opt.subject_osim_model)}")

    # 서버용 기본값 설정
    opt.height_m = 1.75  # 미터
    opt.weight_kg = 70.0  # 킬로그램  
    opt.treadmill_speed = 1.2  # m/s
    
    print(f"\n✅ 설정 완료:")
    print(f"   - .mot 파일: {len(file_paths)}개")
    print(f"   - OpenSim 모델: {os.path.basename(opt.subject_osim_model)}")
    print(f"   - 피험자: {opt.height_m}m, {opt.weight_kg}kg")
    print(f"   - 트레드밀 속도: {opt.treadmill_speed}m/s")
