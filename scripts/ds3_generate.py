...
- Inpaint with the pretrained diffusion model, applying speed-scale soft guidance on pelvis_tx
- Keep most kinematics from the reference (mask=1), leave pelvis_tx/ty/tz free to meet target speed
- Write the generated result back to .mot for OpenSim/GIF rendering

Default input is the converted AddBiomechanics sample:
  /home/ghlee/GaitDynamics/previews/ab_Subj06_walk_09.mot

Usage examples:
  python scripts/ds3_generate.py --scale 1.2 --export-mot
  python scripts/ds3_generate.py --target-speed 1.6 --height-m 1.70 --export-mot
"""
import argparse
import os
import sys
...
def make_out_dir(suffix: str = "") -> Path:
    ts = time.strftime('%Y%m%d_%H%M%S')
    name = ts if not suffix else f"{ts}__{suffix}"
    out = PROJECT_ROOT / 'previews' / 'ds3' / name
...
def main():
    ...
    if args.preset == 'ds3':
        # Stronger guidance like figures (more inner steps, longer span)
    ...
    def build_mask_and_guidance_for_window(state_true_np: np.ndarray):
        ...
        if args.preset == 'ds3':
            # In DS3 preset, hard-fix pelvis ty/tz; pelvis tx is fixed unless --free-pelvis-tx
            if pelvis_tx_idx is not None and not args.free_pelvis_tx:
                mask_local[0, :, pelvis_tx_idx] = 1.0
        ...
        if pelvis_tx_idx is not None:
            if args.preset == 'ds3' and abs(cur_norm_vx) > 1e-5:
                ...
                state_true_local[:, pelvis_tx_idx] = torch.tensor(target_norm_vx, dtype=torch.float32).repeat(T)
        if args.preset != 'ds3':
            if pelvis_ty_idx is not None:
        ...
        if args.preset == 'ds3':
            if pelvis_tx_idx is not None:
                ...
                # If pelvis_tx is free, keep strong soft guidance; if fixed, weight doesn't matter
        ...
        return state_true_norm_local, mask_local, value_diff_thd_local, value_diff_weight_local
    ...
    out_dir = make_out_dir(folder_suffix)
    ...
    for i, s in enumerate(starts):
        ...
        state_true_norm, mask, value_diff_thd, value_diff_weight = build_mask_and_guidance_for_window(win_states)
        cond = torch.ones((1, 6), dtype=torch.float32)
        state_pred_list = motion_model.eval_loop(
            opt,
            state_true=state_true_norm,
            masks=mask,
    ...
    # File naming reflecting input, speed condition, and prior
    dur_tag = f"{int(round((len(out_df)-1)/hz))}s"

    base_name = f"{in_base}_{speed_tag}_{prior_tag}_{lora_tag}_{dur_tag}"
